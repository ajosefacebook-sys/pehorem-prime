...
import { Star, Phone, Mail, Shield, Globe } from "lucide-react"
            ...
            <div className="lg:col-span-1">
              <div className="glass-card rounded-2xl p-6 border border-white/10 text-center sticky top-24">
                <div className="relative w-32 h-32 rounded-2xl overflow-hidden mx-auto mb-4 border-2 border-gold/30">
                  <Image src={agent.avatar} alt={agent.name} fill className="object-cover" sizes="128px" onError={(e) => { (e.target as HTMLImageElement).src = '/placeholder.svg' }} />
                </div>
                  ...
                  {featuredProperties.map((property) => (
                    <Card key={property.id}>
                      <CardImage className="h-40">
                        <Image src={property.images[0]} alt={property.title} fill className="object-cover" sizes="(max-width: 768px) 100vw, 50vw" onError={(e) => { (e.target as HTMLImageElement).src = '/placeholder.svg' }} />
