...
import { Star, Phone, Mail, Globe, Award } from "lucide-react"
            ...
            <div className="lg:col-span-1">
              <div className="glass-card rounded-2xl p-6 border border-white/10 text-center sticky top-24">
                <div className="relative w-32 h-32 rounded-2xl overflow-hidden mx-auto mb-4 border-2 border-gold/30">
                  <Image src={dealer.avatar} alt={dealer.name} fill className="object-cover" sizes="128px" onError={(e) => { (e.target as HTMLImageElement).src = '/placeholder.svg' }} />
                </div>
                  ...
                  {featuredVehicles.map((vehicle) => (
                    <Card key={vehicle.id}>
                      <CardImage className="h-40">
                        <Image src={vehicle.images[0]} alt={vehicle.title} fill className="object-cover" sizes="(max-width: 768px) 100vw, 50vw" onError={(e) => { (e.target as HTMLImageElement).src = '/placeholder.svg' }} />
