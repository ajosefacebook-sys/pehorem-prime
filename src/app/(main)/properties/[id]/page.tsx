...
export default function PropertyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    meta.setAttribute("content", property.description.slice(0, 160))
  }, [property])
          ...
          <div className="relative rounded-2xl overflow-hidden bg-dark-200">
            <div className="relative h-[50vh] sm:h-[60vh] lg:h-[70vh]">
              ...
              <div className="absolute top-4 left-4 flex gap-2">
                ...
                <Badge variant="dark">{property.status.replace("-", " ")}</Badge>
              </div>
          ...
          <div className="grid lg:grid-cols-3 gap-8 lg:gap-12">
            <div className="lg:col-span-2 space-y-8">
              <div>
                ...
                <p className="text-white/40 text-sm mt-1 capitalize">{property.type} &middot; {property.status.replace("-", " ")} &middot; {property.city}</p>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {[
                  ...
                  { icon: Maximize, label: "Size", value: `${property.area} ${property.areaUnit}` },
              ...
              <div>
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  {[...property.features, ...property.amenities].map((item) => (
              ...
              <div>
                <h2 className="text-xl font-display font-semibold text-white mb-4">Property Details</h2>
                <div className="grid sm:grid-cols-2 gap-3">
                  <div className="flex items-center justify-between py-2 border-b border-white/5">
                    <span className="text-white/40 text-sm">Property Type</span>
                    <span className="text-white text-sm font-medium capitalize">{property.type}</span>
                  </div>
                  <div className="flex items-center justify-between py-2 border-b border-white/5">
                    <span className="text-white/40 text-sm">Status</span>
                    <span className="text-white text-sm font-medium capitalize">{property.status.replace("-", " ")}</span>
                  </div>
                  <div className="flex items-center justify-between py-2 border-b border-white/5">
                  ...
                  <div className="flex items-center justify-between py-2 border-b border-white/5">
                    ...
                    <span className="text-white text-sm font-medium">{property.state}</span>
                  </div>
                  <div className="flex items-center justify-between py-2 border-b border-white/5">
                    <span className="text-white/40 text-sm">Year Built</span>
                    <span className="text-white text-sm font-medium">{property.yearBuilt ?? "N/A"}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <Card hover={false} className="sticky top-24">
                <div className="p-6 space-y-6">
                  <div className="text-center pb-4 border-b border-white/10">
                    ...
                    <p className="text-white/40 text-sm capitalize">{property.type} &middot; {property.status.replace("-", " ")}</p>
                  </div>

                  <div className="space-y-3">
                    <Link href={`https://wa.me/234800746736?text=I%27m%20interested%20in%20${encodeURIComponent(property.title)}`} target="_blank">
              ...
              <Card hover={false} className="sticky top-[calc(24rem)]">
                <div className="p-6">
                  <h3 className="text-lg font-display font-semibold text-white mb-4">Send Enquiry</h3>
                  {submitted ? (
                    <div className="bg-gold/10 border border-gold/20 rounded-xl p-4 text-center">
                      ...
                      <p className="text-white/60 text-sm mt-1">
                        Your enquiry for <span className="text-gold">{property.title}</span> has been received. We will contact you within 24 hours.
                      </p>
                      <button
