...
export default function EditBlogPost() {
  ...
  useEffect(() => {
    const id = params.id
    Promise.all([
      fetch(`/api/admin/blog/${id}`, { headers: { Authorization: `Bearer ${token}` } }).then((r) => r.json()),
      ...
      setForm({
        ...
        author: post.author, published: post.published, featured: post.featured,
        categoryId: post.categoryId || "", tags: (post.tags || []).join(", "),
        metaTitle: post.metaTitle || "", metaDesc: post.metaDesc || "", keywords: post.keywords || "",
      })
    ...
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-white/40 text-sm mt-1">{form.title}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="glass-card rounded-2xl border border-white/5 p-6 space-y-5">
        ...
        <div className="flex justify-end gap-3 pt-2">
          ...
          <Button type="submit" variant="gold" disabled={saving}>
            <Save className="w-4 h-4 mr-1.5" /> {saving ? "Saving..." : "Update Post"}
          </Button>
        </div>
      </form>
    </div>
